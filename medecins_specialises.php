...
// Requête pour obtenir les spécialités des professionnels, sauf 'Généraliste'
$sql_specialites = "SELECT DISTINCT specialite FROM professionnels WHERE specialite != 'Généraliste'";
...
// Requête pour obtenir les professionnels par spécialité
$medecins = [];
foreach ($specialites as $specialite) {
    $sql_medecins = "SELECT * FROM professionnels WHERE specialite = '$specialite'";
    $result_medecins = $conn->query($sql_medecins);
    if ($result_medecins->num_rows > 0) {
        while ($row = $result_medecins->fetch_assoc()) {
            $medecins[$specialite][] = $row;
        }
    }
}
                ...
                <div class="container-fluid">
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        ...
                        <?php if (isset($_SESSION['connected']) && $_SESSION['connected'] == true): ?>
                            <div class="me-2">
                                <a href="compte.php" class="btn btn-outline-success me-2">Compte</a>
    ...
    <div id="content" class="cover-container d-flex w-100 p-3 mx-auto flex-column">
        ...
        <div id="medecins" class="lead">
            <div class="dropdown mb-3">
                <button class="btn btn-secondary dropdown-toggle" type="button" id="specialityDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                    Sélectionnez une spécialité
                </button>
                <ul class="dropdown-menu" aria-labelledby="specialityDropdown">
                    <?php foreach ($specialites as $specialite): ?>
                        <li><a class="dropdown-item" href="#" data-speciality="<?php echo htmlspecialchars($specialite); ?>"><?php echo htmlspecialchars($specialite); ?></a></li>
                    <?php endforeach; ?>
                </ul>
            </div>
            ...
            <?php if (!empty($specialites)): ?>
                <?php foreach ($specialites as $specialite): ?>
                    <div class="speciality-content" id="speciality-<?php echo htmlspecialchars($specialite); ?>">
                        <h3><?php echo htmlspecialchars($specialite); ?></h3>
                        <ul class="list-group mb-4">
                            <?php if (!empty($medecins[$specialite])): ?>
                                <?php foreach ($medecins[$specialite] as $professionnel): ?>
...
<script>
    $(document).ready(function() {
        $('.dropdown-item').on('click', function() {
            ...
            $('.speciality-content').hide();
            $('#speciality-' + speciality).show();
        });
    });
</script>
